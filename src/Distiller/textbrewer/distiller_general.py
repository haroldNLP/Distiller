...
class GeneralDistiller(BasicDistiller):
    ...
    def compute_loss(self,results_S,results_T):
        ...
        for ith,inter_match in enumerate(self.d_config.intermediate_matches):
            ...
            if loss_type == 'mi':
                intermediate_loss = match_loss(inter_S, inter_T, self.d_config.critic, self.d_config.baseline_fn, self.d_config.alpha, mask_T=inputs_mask_T, mask_S=inputs_mask_S)
            else:
